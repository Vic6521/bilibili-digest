const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      lede:
        "Keys stay in this Chrome profile and are sent only to DeepSeek and Qwen. This open-source extension has no developer server or analytics.",
      aiProvider: "AI provider",
      providerSummaryLabel: "Supported AI providers",
      ...
      qwenHelpSuffix: ".",
      privacyNote:
        "When you use AI features, DeepSeek receives the video transcript and relevant video context. Qwen receives screenshots only when you ask for visual summaries. Review each provider's terms and pricing before saving.",
      ...
      migrationWarning:
        "Custom provider settings were removed safely. Enter a DeepSeek API key to continue.",
      saving: "Saving…",
    ...
    "zh-CN": {
      ...
      lede:
        "密钥仅保存在当前 Chrome 个人资料中，只会发送给 DeepSeek 和 Qwen。本开源扩展没有开发者服务器，也不使用分析服务。",
      aiProvider: "AI 服务",
      providerSummaryLabel: "支持的 AI 服务",
      providerBadge: "当前版本默认",
      ...
      deepseekHelpSuffix: "。",
      qwenApiKeyLabel: "Qwen API 密钥",
      qwenHelp:
        "Bilibili Digest 使用 Qwen VL Plus 识别截图内容并生成视觉摘要。",
      qwenLink: "创建 Qwen API 密钥",
      qwenHelpSuffix: "。",
      privacyNote:
        "使用 AI 功能时，DeepSeek 会收到视频字幕及相关视频上下文。仅在需要视觉摘要时，Qwen 才会收到截图。保存前请查看各自服务的条款和价格。",
      saveSettings: "保存设置",
      localRemix: "本地改造",
      ...
      customizationPrompt:
        "请把当前本地 Bilibili Digest 工作区改为使用 [PROVIDER] 提供的 [MODEL]。只在当前工作区中操作。编辑前，先确认其中包含 manifest.json，且 manifest 中的 name 是 Bilibili Digest。如果验证失败，请停止，并让我在编程 Agent 中打开 Bilibili Digest 解压后的项目文件夹。不要搜索其他文件夹，不要编辑猜测的副本，不要假设安装路径，也不要声称 Chrome 可以显示操作系统中的绝对源码路径。更新该服务的 API endpoint、请求格式和最少的 Chrome host permissions。保留用户自带密钥模式和 Chrome 本地存储。不要把 API 密钥写入源代码、提交记录、日志、截图、这段提示词或聊天；代码准备好后，请告诉我应该在哪里自行填写密钥。DeepSeek 专用的请求参数和重试逻辑继续只用于 DeepSeek。新服务的专属规则请单独处理，避免相互影响。更新 README.md、README.zh-CN.md、PRIVACY.md、SECURITY.md 和测试。运行 npm test、npm run check 和 npm run package。最后，说明如何重新加载已解压的扩展，并在真实 Bilibili 视频上测试。",
      ...
      migrationWarning:
        "已安全移除自定义服务设置。请输入 DeepSeek API 密钥以继续使用。",
      saving: "正在保存…",
      addDeepseekKey: "请添加 DeepSeek API 密钥。",
      addQwenKey: "请添加 Qwen API 密钥。",
      saved: "已保存。请重新打开 Bilibili Digest 以使用这些设置。",
      ...
      allDataDeleted: "已删除全部 Bilibili Digest 数据。",
  ...
  function initialize(root = globalThis) {
    ...
    const qwenApiKeyInput = doc.getElementById("qwenApiKey");
    ...
    async function loadSettings() {
      try {
        ...
        qwenApiKeyInput.value = settings.qwenApiKey;
    ...
    async function saveSettings(event) {
      ...
      const settings = settingsApi.normalize({
        ...
        qwenApiKey: qwenApiKeyInput.value,
      });
      ...
      if (!settings.qwenApiKey) {
        setStatus(saveStatus, "addQwenKey");
        return;
      }
