...
1. 打开一个标准 B 站视频页面（`bilibili.com/video/...`）或 YouTube 播放页面（`youtube.com/watch?...`）。
2. 点击 Bilibili Digest 扩展图标，打开侧边栏。
3. 阅读带时间戳的字幕，或选择 **原文**、**中文**、**双语**。
4. 需要 AI 生成的章节和重点引用时，打开 **总览**。
5. 选中字幕文本，可以获取 AI 讲解。
6. 在播放器中保存笔记，或从重点引用保存，之后在 **笔记** 中回顾。在视频页面上，点击播放器附近的 **📝 笔记** 按钮，或在视频聚焦时按下 `n` 键，即可保存带截图的时间戳笔记。
7. 需要打印版学习材料时，从 **笔记** 标签页把笔记导出为 PDF。

## 当前可用的功能

- Google Chrome 116 或更高版本，使用 Side Panel API。
- 标准 B 站 `bilibili.com/video` 页面和 `youtube.com/watch` 播放页面。
- B 站原生字幕轨道直接从 B 站字幕 API 获取，Supadata 作为备用。
- YouTube 原生字幕轨道由 Supadata 返回。Bilibili Digest 优先选择中文字幕，但也可能显示其他可用的原生语言。
- 原文、简体中文、中英双语对照字幕视图。
- AI 概览、选中文本讲解、翻译和自动笔记润色。
- 带字幕和截图的时间戳笔记，以及 PDF 导出。
- 本地笔记，以及最近字幕和概览结果的本地缓存。
- 所有文本 AI 功能使用 DeepSeek V4 Flash，截图相关功能使用 Qwen VL Plus。其他服务商需要本地代码适配，本发布版本不支持。

短视频、直播、私有或受限视频，以及没有可用原生字幕的视频可能无法使用。Firefox、Safari、移动浏览器和其他 Chromium 浏览器目前未测试，也不提供支持。

获取 YouTube 字幕时，Bilibili Digest 强制使用 Supadata 的 `mode=native`。它不会请求 AI 生成字幕，也不会在没有原生字幕时进行本地音频转写。
