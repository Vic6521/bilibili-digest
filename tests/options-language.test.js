...
test("Settings copy covers English and Simplified Chinese", () => {
  assert.equal(options.translate("en", "pageTitle"), "Bilibili Digest Settings");
  assert.equal(options.translate("zh-CN", "pageTitle"), "Bilibili Digest 设置");
  assert.equal(options.translate("en", "saveSettings"), "Save settings");
  assert.equal(options.translate("zh-CN", "saveSettings"), "保存设置");
  assert.equal(options.translate("en", "addQwenKey"), "Add a Qwen API key.");
  assert.equal(options.translate("zh-CN", "addQwenKey"), "请添加 Qwen API 密钥。");
  assert.equal(
