...
function loadBackgroundHelpers({
  ...
  const sandbox = {
    ...
    YTD_SETTINGS: {
      ...
      qwenChatCompletionsUrl: () => "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions",
      selectModel: (taskName) => ({ provider: "deepseek", model: "deepseek-v4-flash" }),
      getModelConfig: (taskName, overrides = {}) => ({
        provider: overrides.provider || "deepseek",
        model: overrides.model || "deepseek-v4-flash",
      }),
      getProviderEndpoint: (provider) => provider === "qwen"
        ? "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
        : "https://api.deepseek.com/chat/completions",
      getProviderApiKey: (settings, provider) =>
        provider === "qwen" ? (settings.qwenApiKey || "") : (settings.aiApiKey || ""),
    },
  };
  sandbox.globalThis = sandbox;
